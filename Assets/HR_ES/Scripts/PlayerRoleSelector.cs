...
public class PlayerRoleSelector : MonoBehaviourPunCallbacks
{
    ...
    {
        // --- [이 부분이 추가/수정됩니다] ---
        // 방장(Master Client)은 씬에 들어왔을 때 모든 플레이어의 역할을 초기화시킬 책임이 있습니다.
        // 이는 '계속하기'를 통해 같은 방 멤버 그대로 돌아왔을 때 이전 판의 역할이 남아있는 것을 방지합니다.
    ...
    // --- [이 함수를 스크립트 내부에 새로 추가해주세요] ---
    /// <summary>
    /// 방에 있는 모든 플레이어의 "Role" 커스텀 프로퍼티를 null로 설정합니다.
    /// 마스터 클라이언트만 호출해야 합니다.
    /// </summary>
    void ResetAllPlayerRoles()
    {
        Debug.Log("방장이 모든 플레이어의 역할을 초기화합니다.");
        var props = new ExitGames.Client.Photon.Hashtable { { "Role", null } };
        foreach (var player in PhotonNetwork.PlayerList)
        {
            // 역할이 이미 null이 아닌 경우에만 속성 변경을 요청하여 불필요한 네트워크 트래픽을 줄입니다.
            if (player.CustomProperties.ContainsKey("Role") && player.CustomProperties["Role"] != null)
            {
    ...
    private void Update()
    {
        ...
        if (PhotonNetwork.IsMasterClient && Input.GetKeyDown(KeyCode.P))
        {
            ...
            PhotonNetwork.LoadLevel("GameSpace"); // 실제 게임 씬 이름에 맞게 수정
        }
    }
    ...
    {
        return player.CustomProperties.TryGetValue("Role", out object roleValue) && roleValue != null;
    }
