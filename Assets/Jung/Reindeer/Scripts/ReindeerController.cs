...
public class ReindeerController : MonoBehaviour, IPunObservable, IInteractable
{
    ...
    void Start()
    {
        ...
        {
            // UIManager로부터 순록용 캔버스를 가져옵니다.
            Transform canvasTransform = UIManager.instance.reindeerCanvas.transform;
            ...
            {
                interactionText = interactionUIGroup.transform.Find("Interact_Text")?.GetComponent<TextMeshProUGUI>();
                interactionSlider = interactionUIGroup.transform.Find("Interact_Hold_Slider")?.GetComponent<Slider>();
            }

            if (recoveryUIGroup != null)
            {
                recoveryText = recoveryUIGroup.transform.Find("Recovery_Text")?.GetComponent<TextMeshProUGUI>();
                recoverySlider = recoveryUIGroup.transform.Find("Hold_Slider")?.GetComponent<Slider>();
                if (recoverySlider != null)
                {
                    recoverySliderFill = recoverySlider.transform.Find("Fill Area/Fill")?.GetComponent<Image>();
                }
            }

            if (inventoryUIGroup != null)
            {
                useItemPromptUI = inventoryUIGroup.transform.Find("UseItem_Text")?.GetComponent<TextMeshProUGUI>();
                inventoryUI = inventoryUIGroup.GetComponent<InventoryUI>();
                if (inventoryUI != null)
                {
                    UIManager.instance.SetInventory(inventory, inventoryUI); // UIManager를 통해 인벤토리 연결
                }
            }
    ...
    private void EquipItemFromSlot(int slotIndex)
    {
        ...
        {
            inventoryUI.UpdateSelection(slotIndex);
        }
    }
    ...
    private void UnequipItem()
    {
        ...
        // 3. UI 업데이트 로직을 실행합니다.
        if (UIManager.instance != null && inventoryUI != null)
        {
            inventoryUI.UpdateSelection(-1);
        }
